...
CSS_PATH = os.path.join(FRONTEND_DIR, 'css', 'premium_theme.css')
JS_PATH = os.path.join(FRONTEND_DIR, 'js', 'premium_theme.js')
ANALYTICS_PATH = os.path.join(PAGES_DIR, 'manager_analytics.html')

# 1. Extract CSS from manager_analytics.html
with open(ANALYTICS_PATH, 'r', encoding='utf-8') as f:
    html = f.read()

style_match = re.search(r'<style>(.*?)</style>', html, re.DOTALL)
if style_match:
    premium_css = style_match.group(1).strip()
    with open(CSS_PATH, 'w', encoding='utf-8') as f:
        f.write(premium_css)
else:
    print("Could not find <style> block in manager_analytics.html")
    exit(1)

# 2. Create premium_theme.js
js_content = """
...
for filepath in html_files:
    ...
    if 'premium_theme.css' not in html:
        # Inject CSS
        rel_css = os.path.relpath(CSS_PATH, os.path.dirname(filepath))
        # Find </head> and insert before
        html = html.replace('</head>', f'    <link href="{rel_css}" rel="stylesheet"/>\n</head>')
        modified = True
        
    if 'premium_theme.js' not in html:
        # Inject JS
        rel_js = os.path.relpath(JS_PATH, os.path.dirname(filepath))
        # Find </body> and insert before
        html = html.replace('</body>', f'    <script src="{rel_js}"></script>\n</body>')
        modified = True
...
print("Successfully generated premium theme files and injected them into all HTML pages.")
